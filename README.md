...
```bash
uvx --from copier copier update --trust --vcs-ref HEAD --skip-answered --data project_name=range_discombobulator
```
...
"$tmpdir/venv/bin/copier" copy --trust --vcs-ref HEAD --overwrite \
...
```shell
uvx --from copier copier update --trust --vcs-ref HEAD
```
